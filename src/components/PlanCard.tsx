...
//   const [selectedPrice, setSelectedPrice] = useState<"Monthly" | "Yearly">(
//     "Monthly"
//   );

//   // const handleCancelUpgradePlans = () => setIsUpgrade(false);

//   const PlansCard = ({ plan }: { plan: "Monthly" | "Yearly" }) => {
//     return (
//       <button
...
// Define the interface for pricing
      ...
      <div className="relative w-full h-fit flex xs:flex-col lg:flex-row justify-center mt-10 md:flex-col xs:justify-center xs:items-center md:align-center md:items-center py-10">
        {pricing?.map((p: PricingPlan) => {
          ...
          return (
            ...
            >
              <div className={`absolute -top-[7vw] left-[5vw]`}>
                {p.title === "Starter" ? (
                  <Image src={starter} alt="Starter" className="w-[11vw]" />
                ) : p.title === "Growth" ? (
                  <Image src={growth} alt="Growth" className="w-[11vw]" />
                ) : p.title === "Magnum" ? (
                  <Image src={magnum} alt="Magnum" className="w-[11vw]" />
                ) : null}
              </div>
              <div className="w-full h-full rounded-[1.2vw] md:px-[1vw] md:py-[1.2vw] xs:py-[5vw] xs:px-[3vw]">
                <h3
                ...
                >
                  <span className="md:text-[2vw] xs:text-[5vw] font-bold text-white font-Inter">
                    {p.custom
                      ? "Custom"
                      : p[selectedPrice.toLowerCase() as "monthly" | "yearly"]
                          ?.price}
                  </span>
                ...
                <div className="flex flex-row font-medium font-Inter text-white pt-[1vw] md:text-[1vw] xs:text-[4vw] w-full items-center">
                  <Image
                    src={
                      p[selectedPrice === "Monthly" ? "monthly" : "yearly"]
                        ?.tick || "" // Explicitly specify the keys to ensure TypeScript knows which keys are valid
                    }
                    alt="icon"
                    className="lg:w-[3vw] md:w-[20px] xs:w-[16px]"
                  />
                ...
                <div className="xs:text-[4vw] w-full flex flex-row font-Inter font-medium text-white pt-[0.5vw] pb-[1.5vw] border-b-[0.08vw] border-[#6940F2] md:text-[1vw]">
                  <Image
                    src={
                      p[selectedPrice === "Monthly" ? "monthly" : "yearly"]
                        ?.tick1 || "" // Explicitly map to the correct key
                    }
                    alt="icon"
                    className="lg:w-[3vw] md:w-[18px] xs:w-[16px]"
                  />
                ...
                <p className="w-full h-[5vh] font-Inter font-medium md:text-[1.3vw] xs:text-[3vw] lg:text-[0.789vw] text-[#616874] mt-[2vw]">
                  {p.content}
                </p>
                <div className="h-[10vh] flex items-center gap-5">
                  <button className="getButton get_button">Get Started</button>
